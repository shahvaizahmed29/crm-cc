...
@section('content')
<x-page-header title="Edit Card — {{ $lead->fullName() }}">
    <x-slot:actions>
...
<form action="{{ route('leads.cards.update', [$lead, $card]) }}" method="POST" class="js-card-form mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
    @csrf
    @method('PUT')
    <x-form-field label="Bank Name" for="bank_name">
        <x-input id="bank_name" name="bank_name" type="text" :value="old('bank_name', $card->bank_name)" />
    </x-form-field>
    <x-form-field label="Bank Tollfree" for="bank_tollfree">
        <x-input id="bank_tollfree" name="bank_tollfree" type="text" :value="old('bank_tollfree', $card->bank_tollfree)" />
    </x-form-field>
    <x-form-field label="Credit Card Number" for="card_number">
        <x-input id="card_number" name="card_number" type="text" :value="old('card_number', $card->card_number)" />
    </x-form-field>
    <x-form-field label="Name On Card" for="name_on_card">
        <x-input id="name_on_card" name="name_on_card" type="text" :value="old('name_on_card', $card->name_on_card)" />
    </x-form-field>
    <x-form-field label="Card Expiry (MM/YY)" for="card_expiry">
        <x-input id="card_expiry" name="card_expiry" type="text" :value="old('card_expiry', $card->card_expiry)" />
    </x-form-field>
    <x-form-field label="CVC" for="card_cvc">
        <x-input id="card_cvc" name="card_cvc" type="text" :value="old('card_cvc', $card->card_cvc)" />
    </x-form-field>
    <x-form-field label="Balance $" for="balance">
        <x-input id="balance" name="balance" type="number" :value="old('balance', $card->balance)" step="0.01" min="0" />
    </x-form-field>
    <x-form-field label="Available $" for="available_amount">
        <x-input id="available_amount" name="available_amount" type="number" :value="old('available_amount', $card->available_amount)" step="0.01" min="0" />
    </x-form-field>
    <x-form-field label="Last Payment" for="last_payment">
        <x-input id="last_payment" name="last_payment" type="text" :value="old('last_payment', $card->last_payment)" />
    </x-form-field>
    <x-form-field label="Due Payment" for="due_payment">
        <x-input id="due_payment" name="due_payment" type="text" :value="old('due_payment', $card->due_payment)" />
    </x-form-field>
    <x-form-field label="APR %" for="apr">
        <x-input id="apr" name="apr" type="number" :value="old('apr', $card->apr)" step="0.01" min="0" />
    </x-form-field>
    <x-form-field label="Fees $" for="fees">
        <x-input id="fees" name="fees" type="number" :value="old('fees', $card->fees)" step="0.01" min="0" />
    </x-form-field>
    <div class="flex items-center gap-2">
        <input
            ...
            id="charge_card"
            value="1"
            {{ old('charge_card', $card->charge_card) ? 'checked' : '' }}
    ...
    <div class="lg:col-span-2">
        <x-form-field label="Comment" for="comment">
            <x-input id="comment" name="comment" type="textarea" :value="old('comment', $card->comment)" :rows="3" />
        </x-form-field>
    </div>

    <div class="lg:col-span-2">
        <button type="submit" class="rounded-md bg-sky-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-sky-500">Update Card</button>
    </div>
</form>

<form action="{{ route('leads.cards.destroy', [$lead, $card]) }}" method="POST" class="mt-3" onsubmit="return confirm('Delete this card?');">
    @csrf
    @method('DELETE')
    <button type="submit" class="rounded-md bg-red-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-red-500">Delete Card</button>
</form>
